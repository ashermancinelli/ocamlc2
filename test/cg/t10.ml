 ...
 * CHECK:           %[[VAL_0:.*]] = ocaml.closure.env.new {for = "testenv$0"}
 * CHECK:           %[[VAL_1:.*]] = ocaml.unit
 * CHECK:           %[[VAL_2:.*]] = arith.constant 7 : i64
 * CHECK:           %[[VAL_3:.*]] = ocaml.convert %[[VAL_2]] from i64 to !ocaml.box<i64>
 * CHECK:           %[[VAL_4:.*]] = call @test(%[[VAL_3]]) : (!ocaml.box<i64>) -> !ocaml.sbox
 * CHECK:           %[[VAL_5:.*]] = call @print_string(%[[VAL_4]]) : (!ocaml.sbox) -> !ocaml.unit
